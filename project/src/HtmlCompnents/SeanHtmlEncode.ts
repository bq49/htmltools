...
class SeanHtmlEncode
{
    public static obj:SeanHtmlEncode = null;
    public confObj:any;

    public static getInstance(confObj:any):any
    {
    ...
    public encodeHtml2Obj(str_html:string):any
    {
        //一段带html标签的文本
        //去掉所有html标签，获取完整字符内容
    ...
    //遍历字符串，提取节点
    public foreachStyles(parentNode:any,start:number,str_html:string):any
    {
        var return_nodes = [];
        //逐个字符串检查标签
        var startAt = start;
    ...
    //查找下个节点在字符串中的位置
    public getNextNodePosition(start:number,str_html:string):number
    {
        //所有支持的节点
    ...
    public getNodeFrom(parentNode:any,start:number,str_html:string):any
    {
        ...
        if( !curNode )
        {
            ...
            if( p >= 0 )
            {
                ...
                if( str_null.length > 0 )
                {
                    ...
                    //     "end":p
                    // };
                    var b = new SeanAttributeNode(
                        "",
                        this.getDefaultAttr(parentNode),
                        str_null,
                        false,
                        p
                    );
        ...
        // };
        var a = new SeanAttributeNode(
            curNode.key,
            attr,
            html,
            bChildren,
            end+curNode.end.length
        );
    ...
    //查到之后，返回一个属性数组
    public checkAttrNodeEnd(str_html:string,start:number,end:number):any
    {
        var attrs = [];
    ...
    public indexOfCount(key:string,str_html:string,start:number,num:number):number
    {
    ...
    public getCountOf(key:string,start:number,end:number,str_html:string):number
    {
        var a = 0;
    ...
    //是否有子节点
    public isHaveChildren(html:string):boolean
    {
        var libNodes = this.confObj.labels;
    ...
    //获取缺省属性
    public getDefaultAttr(parentNode:any):any
    {
        var attr = [];
        if( parentNode )
        {
    ...
    //提取节点属性
    public getAttrByNode(curNode:any,htmls:string):any
    {
        //先对属性内容进行过滤
        var html = htmls;
        ...
        for( var i = 0; i < attrNode.length; i++ )
        {
            ...
            var pos:number = curNode.lb.indexOf(attrN,0);
            if( pos == 0 )
            {
                //与该节点匹配了
                //提取该节点属性及值
    ...
    public rebuildAttrValue(attr:string,value:any):string
    {
        var a = attr+"-"+value;
    ...
    public getAttrValue(attrName:string,str:string):any
    {
    ...
    //过滤所有html标签
    public filterHtmls(str_html):string
    {
